var AWOnestepcheckoutCore = {
    ...
    validateParams: function(params) {
        ...
        var regExp = new RegExp(matchers.join("|"), "i");
        for (var key in params) {
            if (key = 'login[password]') {
                continue;
            }
            var input = $$('[name="' + key + '"]');
            this.modifyInput(input, true);
            var valueToCheck = params[key];
            if (params[key] instanceof Array) {
                valueToCheck = valueToCheck.join(",");
            }
            if (regExp.test(valueToCheck)) {
                this.modifyInput(input, false);
                isValid = false;
            }
        }
        return isValid;
    ...
    modifyInput: function(input, remove) {
        if (!input.length) {
            return;
        }
        if (remove) {
            if (input.length > 1) {
                ...
                input.first().removeClassName('validation-failed');
            }
        } else {
            if (input.length > 1) {
                input.each(function(el) {
                ...
                input.first().addClassName('validation-failed');
            }
        }
    }
};
